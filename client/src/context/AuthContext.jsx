...
export const AuthProvider = ({ children }) => {
  ...
  const [session, setSession] = useState(() =>
    parseSession(readStorage(SESSION_KEY)),
  );
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      try {
        ...
        // ✅ Only clear auth if the token is actually invalid (401).
        // ✅ Don't wipe session on transient issues (network/CORS/5xx) because it breaks refresh persistence.
        if (error?.status === 401) {
  ...
  }, [token, session?.school, clearAuth]);
